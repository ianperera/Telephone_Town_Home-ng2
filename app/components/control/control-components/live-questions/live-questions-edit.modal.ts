...
export class LiveQuestionModalContext extends BSModalContext {
    id: number;
...
@Component({
    selector: 'app-live-question-edit-modal',
    styles: [
...
export class LiveQuestionEditModal implements OnInit, ModalComponent<LiveQuestionModalContext> {
    context: LiveQuestionModalContext;

    constructor(public dialog: DialogRef<LiveQuestionModalContext>) {
    }

    ngOnInit() {
        this.context = Object.assign({}, this.dialog.context);
        console.log('here is the context', this.context);
    }
